...
from custom_components.emby.maintenance_store import (
    EmbiMaintenanceStore,
    resolve_store_load,
)
...
@pytest.mark.asyncio
async def test_empty_backend_remains_distinguishable_from_initialized_state() -> None:
...
@pytest.mark.asyncio
async def test_missing_or_corrupt_expected_store_fails_safe() -> None:
    loaded = await EmbiMaintenanceStore(FakeBackend()).async_load()
    decision = resolve_store_load(
        loaded,
        store_expected=True,
...
@pytest.mark.asyncio
async def test_store_round_trip_preserves_report_and_scheduler() -> None:
    ...
    decision = resolve_store_load(
        restored,
        store_expected=True,
        legacy_initial_run_completed=False,
    )
    assert decision.storage_available is True
    assert decision.initialize_store is False
...
@pytest.mark.asyncio
async def test_store_errors_propagate_for_fail_safe_runtime_handling() -> None:
...
def test_persistent_report_contains_no_device_id_fields() -> None:
    serialized = MaintenanceState(report=CleanupRunReport(server_deleted=5)).as_dict()
