...
def test_dirty_draft_blocks_critical_actions() -> None:
    registry = source("options_registry.py")
    maintenance = source("maintenance_flow.py")

    assert 'if self._dirty:\n            return self.async_abort(reason="unsaved_changes")' in registry
    assert maintenance.count('return self.async_abort(reason="unsaved_changes")') >= 1


def test_automatic_cleanup_warning_has_boolean_confirmation_without_phrase() -> None:
    maintenance = source("maintenance_flow.py")
